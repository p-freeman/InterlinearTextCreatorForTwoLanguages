...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        # Create line number display for English and German text fields
        self.english_line_numbers = LineNumbers(self.frame, width=4, bg="#f0f0f0", bd=0, wrap="none")
        self.english_line_numbers.grid(row=3, column=2, sticky="nsew")
        ...
        self.label_english = tk.Label(self.frame, text="Englische Wörter:")
        self.label_english.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_english = tk.Text(self.frame, height=10, width=30)
        self.entry_english.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_english.bind("<Button-1>", self.scroll_german_to_same_line)
    ...
    def create_interlinear(self):
        ...
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_english.get("1.0", tk.END).strip() or self.entry_german.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
        ...
        # Save English and German text to text files with Windows-compatible line breaks
        ...
        interlinear_html += ".english_word { font-weight: bold; }\n"
        ...
        # Iterate through English and German lines to create interlinear pairs
        for english_line, german_line in zip(english_lines, german_lines):
            # Handle paragraphs
    ...
    def scroll_german_to_same_line(self, event):
        """
        Scroll the German text field to the same line as the clicked line in the English text field.
        """
        index = self.entry_english.index(tk.CURRENT)

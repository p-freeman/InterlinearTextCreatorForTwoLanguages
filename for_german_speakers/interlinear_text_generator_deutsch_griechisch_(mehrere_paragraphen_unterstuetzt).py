...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        # Create line number display for Greek and German text fields
        self.greek_line_numbers = LineNumbers(self.frame, width=4, bg="#f0f0f0", bd=0, wrap="none")
        self.greek_line_numbers.grid(row=3, column=2, sticky="nsew")
        ...
        self.label_greek = tk.Label(self.frame, text="Griechische Wörter:")
        self.label_greek.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_greek = tk.Text(self.frame, height=10, width=30)
        self.entry_greek.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_greek.bind("<Button-1>", self.scroll_german_to_same_line)
    ...
    def create_interlinear(self):
        ...
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_greek.get("1.0", tk.END).strip() or self.entry_german.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
        ...
        greek_text = self.entry_greek.get("1.0", tk.END).strip()
        ...
        # Save Greek and German text to text files with Windows-compatible line breaks
        greek_text_path = os.path.join(folder_path, "greek_text.txt")
        with open(greek_text_path, "w", encoding="utf-8") as file:
            file.write(greek_text.strip('\n'))
        ...
        # Read Greek and German text files
        with open(greek_text_path, "r", encoding="utf-8") as file:
            greek_lines = file.readlines()
        ...
        interlinear_html += ".greek_word { font-weight: bold; }\n"
        ...
        # Iterate through Greek and German lines to create interlinear pairs
        for greek_line, german_line in zip(greek_lines, german_lines):
            # Handle paragraphs
            if greek_line.strip() == "":
                interlinear_html += "<br>\n"
                continue
            ...
            interlinear_html += f"<div class='greek_word'>{greek_line.strip()}</div>\n"
        ...
        print("Greek text saved to:", greek_text_path)
    ...
    def open_project(self):
        ...
        self.entry_greek.delete("1.0", tk.END)
        ...
        if os.path.exists(json_file_path):
            ...
            self.entry_greek.insert("1.0", project_data.get("greek_text", ""))
    ...
    def scroll_german_to_same_line(self, event):
        """
        Scroll the German text field to the same line as the clicked line in the Greek text field.
        """
        index = self.entry_greek.index(tk.CURRENT)

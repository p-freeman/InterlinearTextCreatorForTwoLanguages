...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        # Create line number display for Italian and German text fields
        self.italian_line_numbers = LineNumbers(self.frame, width=4, bg="#f0f0f0", bd=0, wrap="none")
        self.italian_line_numbers.grid(row=3, column=2, sticky="nsew")
        ...
        self.label_italian = tk.Label(self.frame, text="Italienische Wörter:")
        self.label_italian.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_italian = tk.Text(self.frame, height=10, width=30)
        self.entry_italian.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_italian.bind("<Button-1>", self.scroll_german_to_same_line)
    ...
    def create_interlinear(self):
        ...
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_italian.get("1.0", tk.END).strip() or self.entry_german.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
        ...
        italian_text = self.entry_italian.get("1.0", tk.END).strip()
        ...
        # Save Italian and German text to text files with Windows-compatible line breaks
        italian_text_path = os.path.join(folder_path, "italian_text.txt")
        with open(italian_text_path, "w", encoding="utf-8") as file:
            file.write(italian_text.strip('\n'))
        ...
        # Read Italian and German text files
        with open(italian_text_path, "r", encoding="utf-8") as file:
            italian_lines = file.readlines()
        ...
        interlinear_html += ".italian_word { font-weight: bold; }\n"
        ...
        # Iterate through Italian and German lines to create interlinear pairs
        for italian_line, german_line in zip(italian_lines, german_lines):
            # Handle paragraphs
            if italian_line.strip() == "":
                interlinear_html += "<br>\n"
                continue
            
            interlinear_html += "<div class='word_package'>\n"
            interlinear_html += f"<div class='italian_word'>{italian_line.strip()}</div>\n"
        ...
        print("Interlinearer Text gespeichert unter:", file_path)
        print("Italienischer Text gespeichert unter:", italian_text_path)
        print("Deutscher Text gespeichert unter:", german_text_path)
    ...
    def open_project(self):
        ...
        self.entry_italian.delete("1.0", tk.END)
        ...
        if os.path.exists(json_file_path):
            ...
            self.entry_italian.insert("1.0", project_data.get("italian_text", ""))
    ...
    def scroll_german_to_same_line(self, event):
        """
        Scroll the German text field to the same line as the clicked line in the Italian text field.
        """
        index = self.entry_italian.index(tk.CURRENT)

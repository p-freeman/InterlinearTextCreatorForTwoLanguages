...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        # Create line number display for Spanish and German text fields
        self.spanish_line_numbers = LineNumbers(self.frame, width=4, bg="#f0f0f0", bd=0, wrap="none")
        self.spanish_line_numbers.grid(row=3, column=2, sticky="nsew")
        ...
        self.label_spanish = tk.Label(self.frame, text="Spanische Wörter:")
        self.label_spanish.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_spanish = tk.Text(self.frame, height=10, width=30)
        self.entry_spanish.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_spanish.bind("<Button-1>", self.scroll_german_to_same_line)
    ...
    def create_interlinear(self):
        ...
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_spanish.get("1.0", tk.END).strip() or self.entry_german.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
        ...
        spanish_text = self.entry_spanish.get("1.0", tk.END).strip()
        ...
        # Save Spanish and German text to text files with Windows-compatible line breaks
        spanish_text_path = os.path.join(folder_path, "spanish_text.txt")
        with open(spanish_text_path, "w", encoding="utf-8") as file:
            file.write(spanish_text.strip('\n'))
        ...
        # Read Spanish and German text files
        with open(spanish_text_path, "r", encoding="utf-8") as file:
        ...
        interlinear_html += ".spanish_word { font-weight: bold; }\n"
        ...
        # Iterate through Spanish and German lines to create interlinear pairs
        for spanish_line, german_line in zip(spanish_lines, german_lines):
            # Handle paragraphs
            if spanish_line.strip() == "":
                interlinear_html += "<br>\n"
                continue
            
            interlinear_html += "<div class='word_package'>\n"
            interlinear_html += f"<div class='spanish_word'>{spanish_line.strip()}</div>\n"
        ...
        print("Spanish text saved to:", spanish_text_path)
    ...
    def open_project(self):
        ...
        self.entry_spanish.delete("1.0", tk.END)
        ...
        if os.path.exists(json_file_path):
            ...
            self.entry_spanish.insert("1.0", project_data.get("spanish_text", ""))
    ...
    def scroll_german_to_same_line(self, event):
        """
        Scroll the German text field to the same line as the clicked line in the Spanish text field.
        """
        index = self.entry_spanish.index(tk.CURRENT)

...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        self.label_swissgerman = tk.Label(self.frame, text="Swissgerman Words:")
        ...
        self.entry_swissgerman.bind("<Button-1>", self.scroll_polish_to_same_line)
        
        self.label_polish = tk.Label(self.frame, text="Polish Words:")
        self.label_polish.grid(row=2, column=1, padx=5, pady=5, sticky="w")
        self.entry_polish = tk.Text(self.frame, height=10, width=30)
        self.entry_polish.grid(row=3, column=1, padx=5, pady=5, sticky="nsew")
        ...
        self.label_video_link = tk.Label(self.frame, text="Link to Video (URL) *optional:")
        ...
        self.audio_checkbox = tk.Checkbutton(self.frame, text="Audio-File available *optional", variable=self.audio_checkbox_var)
    ...
    def create_interlinear(self):
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_swissgerman.get("1.0", tk.END).strip() or self.entry_polish.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
            return  
        ...
        interlinear_html += ".polish_word {{ margin-top: 5px; }}\n"  
        interlinear_html += "</style>\n</head>\n<body style='font-size:" + str(font_size) + "px;'>\n"  
        
        interlinear_html += f"<h1>{title}</h1>\n"  
        
        for swissgerman_line, polish_line in zip(swissgerman_lines, polish_lines):
            ...
            interlinear_html += f"<div class='polish_word'>{polish_line.strip()}</div>\n"  
    ...
    def scroll_polish_to_same_line(self, event):
        index = self.entry_swissgerman.index(tk.CURRENT)
        line_number = int(index.split(".")[0])
        self.entry_polish.see(f"{line_number}.0")

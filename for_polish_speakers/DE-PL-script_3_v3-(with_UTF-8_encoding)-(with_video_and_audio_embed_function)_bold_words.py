...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        self.label_filename = tk.Label(self.frame, text="Nazwa pliku / Nazwa folderu:")
        ...
        self.label_german = tk.Label(self.frame, text="Niemieckie Słowa:")
        self.label_german.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_german = tk.Text(self.frame, height=10, width=30)
        self.entry_german.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_german.bind("<Button-1>", self.scroll_polish_to_same_line)
        
        self.label_polish = tk.Label(self.frame, text="Polskie Słowa:")
        ...
        self.label_video_link = tk.Label(self.frame, text="Link do Wideo (URL) *opcjonalne:")
        ...
        self.audio_checkbox = tk.Checkbutton(self.frame, text="Dostępny plik audio *opcjonalne", variable=self.audio_checkbox_var)
        self.audio_checkbox.grid(row=7, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        
        self.create_button = tk.Button(self.frame, text="Utwórz Tekst Interlinearny", command=self.create_interlinear)
    ...
    def create_interlinear(self):
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_german.get("1.0", tk.END).strip() or self.entry_polish.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
            return  
        ...
        interlinear_html += ".german_word {{ font-weight: bold; }}\n"  
        interlinear_html += ".polish_word {{ margin-top: 5px; }}\n"  
        interlinear_html += "</style>\n</head>\n<body style='font-size:" + str(font_size) + "px;'>\n"  
        
        interlinear_html += f"<h1>{title}</h1>\n"  
        
        for german_line, polish_line in zip(german_lines, polish_lines):
            if german_line.strip() == "":
                interlinear_html += "<br>\n"
                continue
            
            interlinear_html += "<div class='word_package'>\n"  
            interlinear_html += f"<div class='german_word'><b>{german_line.strip()}</b></div>\n"  
    ...
    def scroll_polish_to_same_line(self, event):
        index = self.entry_german.index(tk.CURRENT)

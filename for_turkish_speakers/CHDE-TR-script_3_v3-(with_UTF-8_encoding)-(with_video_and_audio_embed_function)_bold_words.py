...
class InterlinearApp:
    ...
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Interlinear Metin Oluşturucu")
        ...
        self.label_title = tk.Label(self.frame, text="Başlık:")
        ...
        self.label_filename = tk.Label(self.frame, text="Dosya Adı/Klasör Adı:")
        ...
        self.label_swissgerman = tk.Label(self.frame, text="Swissgerman Kelimeler:")
        self.label_swissgerman.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_swissgerman = tk.Text(self.frame, height=10, width=30)
        self.entry_swissgerman.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.entry_swissgerman.bind("<Button-1>", self.scroll_turkish_to_same_line)
        
        self.label_turkish = tk.Label(self.frame, text="Türkçe Kelimeler:")
        self.label_turkish.grid(row=2, column=1, padx=5, pady=5, sticky="w")
        self.entry_turkish = tk.Text(self.frame, height=10, width=30)
        self.entry_turkish.grid(row=3, column=1, padx=5, pady=5, sticky="nsew")
        
        self.label_vertical_distance = tk.Label(self.frame, text="Dikey Mesafe (px):")
        ...
        self.label_font_size = tk.Label(self.frame, text="Yazı Tipi Boyutu:")
        ...
        self.label_video_link = tk.Label(self.frame, text="Video Bağlantısı (URL) *isteğe bağlı:")
        ...
        self.audio_checkbox = tk.Checkbutton(self.frame, text="Ses Dosyası Mevcut *isteğe bağlı", variable=self.audio_checkbox_var)
        self.audio_checkbox.grid(row=7, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        
        self.create_button = tk.Button(self.frame, text="Interlinear Metin Oluştur", command=self.create_interlinear)
        self.create_button.grid(row=8, column=0, padx=5, pady=5)
        
        self.open_button = tk.Button(self.frame, text="Mevcut Projeyi Aç", command=self.open_project)
    ...
    def create_interlinear(self):
        if not (self.entry_title.get() or self.entry_filename.get() or self.entry_swissgerman.get("1.0", tk.END).strip() or self.entry_turkish.get("1.0", tk.END).strip() or self.entry_vertical_distance.get() or self.entry_font_size.get()):
            return  
        
        title = self.entry_title.get().strip()
	        filename = self.entry_filename.get().strip()
        swissgerman_text = self.entry_swissgerman.get("1.0", tk.END).strip()
        turkish_text = self.entry_turkish.get("1.0", tk.END).strip()
        ...
        swissgerman_text_path = os.path.join(folder_path, "swissgerman_text.txt")
        with open(swissgerman_text_path, "w", encoding="utf-8") as file:
            file.write(swissgerman_text.strip('\n'))
        
        turkish_text_path = os.path.join(folder_path, "turkish_text.txt")
        with open(turkish_text_path, "w", encoding="utf-8") as file:
            file.write(turkish_text.strip('\n'))
        
        with open(swissgerman_text_path, "r", encoding="utf-8") as file:
            swissgerman_lines = file.readlines()
        
        with open(turkish_text_path, "r", encoding="utf-8") as file:
            turkish_lines = file.readlines()
        ...
        interlinear_html += ".swissgerman_word {{ font-weight: bold; }}\n"  
        interlinear_html += ".turkish_word {{ margin-top: 5px; }}\n"  
        interlinear_html += "</style>\n</head>\n<body style='font-size:" + str(font_size) + "px;'>\n"  
        
        interlinear_html += f"<h1>{title}</h1>\n"  
        
        for swissgerman_line, turkish_line in zip(swissgerman_lines, turkish_lines):
            if swissgerman_line.strip() == "":
                interlinear_html += "<br>\n"
                continue
            
            interlinear_html += "<div class='word_package'>\n"  
            interlinear_html += f"<div class='swissgerman_word'><b>{swissgerman_line.strip()}</b></div>\n"  
            interlinear_html += f"<div class='turkish_word'>{turkish_line.strip()}</div>\n"  
        ...
        json_data = {
            "title": title,
            "filename": filename,
            "swissgerman_text": swissgerman_text,
        ...
        print("Swissgerman text saved to:", swissgerman_text_path)
        print("Turkish text saved to:", turkish_text_path)
    ...
    def open_project(self):
        ...
        self.entry_swissgerman.delete("1.0", tk.END)
        self.entry_turkish.delete("1.0", tk.END)
        self.entry_vertical_distance.delete(0, tk.END)
        self.entry_font_size.delete(0, tk.END)
        self.entry_video_link.delete(0, tk.END)
        
        project_folder = filedialog.askdirectory(title="Proje Klasörünü Seç")
        ...
        if os.path.exists(json_file_path):
            ...
            self.entry_swissgerman.insert("1.0", project_data.get("swissgerman_text", ""))
            self.entry_turkish.insert("1.0", project_data.get("turkish_text", ""))
            ...
            print("Seçilen klasörde JSON dosyası bulunamadı.")

    def scroll_turkish_to_same_line(self, event):
        index = self.entry_swissgerman.index(tk.CURRENT)
        line_number = int(index.split(".")[0])
        self.entry_turkish.see(f"{line_number}.0")
